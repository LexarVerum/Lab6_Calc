...
class Calculator(QWidget):
    def __init__(self):
        ...
        self.hbox_operation.addWidget(self.b_div)
        self.b_mult = QPushButton("*", self)
        self.hbox_operation.addWidget(self.b_mult)
    ...
    def _operation(self, op):
        ...
        if self.input.text() == '.': self.input.setText("0") #костыль
        self.num_1 = float(self.input.text())
        self.op = op
        self.input.setText("")

    def _result(self):
        if self.input.text()=='.': self.input.setText("")
        if self.input.text() == '': return
        ...
        try:
            if self.op == "/":
                self.input.setText(str(self.num_1 / self.num_2))
        except ZeroDivisionError: self.input.setText("")
        if self.op == "-":
            self.input.setText(str(self.num_1 - self.num_2))
        if self.op == "C":
            self.input.setText('')
            del self.num_1, self.num_2
        self.op = "C"
